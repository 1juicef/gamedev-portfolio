  ...
  - id: D1
    ...
    verification:
      - kind: manual_procedural
        ...
        status: unknown
    human_judgment: true
    rationale: "The original failure was only ever observed in a real browser and is intermittent; it cannot be automated-verified in this session's tooling (video fetches stalled even for a known-good control clip). Concurrent quick task 260728-rrq subsequently identified the actual root cause as a codec-level failure (legacy MPEG-4 Part 2, unsupported by all browsers) and fixed it by re-encoding to H.264 — this plan's timing/buffering hardening alone was never going to resolve that class of failure. See 'Owed to the User' below."
  - id: D2
...
**Update — the actual root cause was found and fixed separately, by quick task 260728-rrq, executed concurrently with this one.** `Crouch walking1.mp4` was encoded as legacy MPEG-4 Part 2 (`codec_name=mpeg4`, `codec_tag_string=mp4v`), a codec no modern browser's `<video>` element decodes — that is why the clip was permanently stuck at a gray box regardless of timing/buffering. 260728-rrq re-encoded the file to H.264 in place (same filename/dimensions, commit `d4afdaf`), which is the fix that actually resolves the reported bug. This plan's autoplay hardening (preload/load()/retry) remains a legitimate, harmless defensive improvement, but it was never going to fix a codec-level failure, and the user should look to 260728-rrq's SUMMARY for the real root-cause fix and its own still-owed real-browser confirmation.

**Process note for the record:** during self-check in this session, an `npm run build` invocation coincided with 260728-rrq's concurrent in-place binary replacement of `Crouch walking1.mp4`, and a `git checkout -- "public/img/other-stuff/Crouch walking1.mp4"` run here (intended to revert what looked like an out-of-scope build side effect) briefly reverted that file to its pre-fix MPEG-4 Part 2 content. This was caught immediately via `ffprobe`, and the H.264 encode was reproduced with the exact ffmpeg command documented in 260728-rrq's plan to restore it; the concurrent session then committed its own fix (`d4afdaf`), and the working tree now matches HEAD exactly with no lasting effect. Flagging this so the user is aware two quick tasks touched overlapping ground in the same window, in case anything about `Crouch walking1.mp4` looks unexpected.
